...
TableMap.prototype.updateColumnInfo = function() {
	...
	console.error('[DEBUG] updateColumnInfo called:', {
  ...
  // Extra check for tableMap
  if (!tableMap) {
    console.error('[DEBUG] ERROR: tableMap not found:', {
			timestamp: new Date().toISOString(),
			tableId: this.tableId,
			schemaName: this.schemaName,
			tableName: this.tableName,
			availableTableIds: Object.keys(this.tableMap)
		})
  }
  const columnSchemas = tableMap.columnSchemas;
  // Another check for columnSchemas
  if (!columnSchemas) {
   	console.error('[DEBUG] ERROR: columnSchemas not found:', {
			timestamp: new Date().toISOString(),
			tableId: this.tableId,
			schemaName: this.schemaName,
			tableName: this.tableName,
			tableMapKeys: Object.keys(tableMap)
		}); 
  }

  if (columnSchemas.length !== this.columnCount) {
    console.error('[DEBUG] ERROR: Column count mismatch in updateColumnInfo:', {
			timestamp: new Date().toISOString(),
			tableId: this.tableId,
			schemaName: this.schemaName,
			tableName: this.tableName,
			expectedCount: this.columnCount,
			actualCount: columnSchemas.length,
			difference: this.columnCount - columnSchemas.length,
			columnNamesFromQuery: columnSchemas.map(c => c.COLUMN_NAME),
			columnTypesFromBinlog: this.columnTypes
		});
  }
  const columns = [];
  for (let j = 0; j < this.columnCount; j++) {
    if (!columnSchemas[j]) {
      console.error('[DEBUG] ERROR: Missing columnSchema at index:', {
				timestamp: new Date().toISOString(),
				index: j,
				tableId: this.tableId,
				schemaName: this.schemaName,
				tableName: this.tableName,
				columnCount: this.columnCount,
				columnSchemasLength: columnSchemas.length
			});
    }
    columns.push({
