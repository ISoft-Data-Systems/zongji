...
// Create a Parser class that mimics the mysql Parser API using mysql2's Packet
class Parser {
  constructor() {
  ...
  parseUnsignedNumber(size) {
    if (size === 1) return this._buffer.readUInt8(this._offset++);
    if (size === 2) {
      this._offset += 2;
    ...
    if (size === 3) {
      const val = this._buffer.readUInt8(this._offset) +
                 (this._buffer.readUInt8(this._offset + 1) << 8) +
                 (this._buffer.readUInt8(this._offset + 2) << 16);
      this._offset += 3;
      return val;
    }
    if (size === 4) {
    ...
    throw new Error('Unsupported size for parseUnsignedNumber: ' + size);
  }
}
